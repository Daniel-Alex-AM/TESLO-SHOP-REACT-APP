...
export const RegisterPage = () => {
  ...
  const handleRegister = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const registerSuccess = await register(email, pwd, fullname);
    if (registerSuccess) {
      navigate('/')
      return
    } else {
      toast.error('Error, pruebe de nuevo más tarde')
    }
  ...
  return (
    <div className={"flex flex-col gap-6"}>
          ...
          <form className="p-6 md:p-8" onSubmit={(e) => { handleRegister(e) }}>
            <div className="flex flex-col gap-6">
              ...
              <div className="grid gap-2">
                ...
                <Input id="password" name="pwd" type="password" required placeholder="Contraseña" />
              </div>
      ...
      <div className="text-balance text-center text-xs text-muted-foreground [&_a]:underline [&_a]:underline-offset-4 hover:[&_a]:text-primary">
        Haciendo click estás de acuerdo con <a href="#">Términos</a> y <a href="#">Políticas de privacidad</a>.
      </div>
    </div>
  )
}
